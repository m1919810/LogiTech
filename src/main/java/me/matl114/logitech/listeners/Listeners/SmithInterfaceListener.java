...
public class SmithInterfaceListener implements Listener {
    ...
    private final ItemStack INTERFACED_NO_RECIPE =
            new CustomItemStack(Material.BARRIER, "&a点击进行合成", "&7该工作方块已成功接入锻铸合成端口", "&c暂无匹配的配方!");
    private final ItemStack INTERFACED_CRAFTABLE = new CustomItemStack(
            Material.CRAFTING_TABLE, "&a点击进行合成", "&7该工作方块已成功接入锻铸合成端口", "&a成功寻找到可行的合成配方", "&7点击后将于接口中输出合成结果");
    private final ItemStack INTERFACED_ANVIL =
            new CustomItemStack(Material.ANVIL, "&a点击进行锻造", "&7该工作方块已成功接入锻铸合成端口", "&a成功寻找到可行的锻造操作", "&7点击后将于接口中输出锻造结果");
    private final ItemStack INTERFACED_SMITH = new CustomItemStack(
            Material.SMITHING_TABLE, "&a点击进行锻造", "&7该工作方块已成功接入锻铸合成端口", "&a成功寻找到可行的锻造操作", "&7点击后将于接口中输出锻造结果");
    private final ItemStack INTERFACED_GRIND = new CustomItemStack(
            Material.GRINDSTONE, "&a点击进行卸载", "&7该工作方块已成功接入锻铸合成端口", "&a成功寻找到可行的卸载方案", "&7点击后将于接口中输出锻造结果");

    public boolean isSupportType(Inventory inventory) {
        return inventory instanceof CraftingInventory
                || inventory instanceof AnvilInventory
                || inventory instanceof SmithingInventory
                || inventory instanceof GrindstoneInventory; /*can add more types*/
    }
    ...
    public void onCloseInventory(InventoryCloseEvent e) {
        ...
        if (isSupportType(inventory) && openingCraftInventory.containsKey(inventory)) {
            ...
            if (re != null
                    && (re.size() >= 2
                            || re.stream().anyMatch(p -> p.getUniqueId()
    ...
    public <T extends Inventory, W extends Object> void onPlayerCraftCommon(
            T inventory,
            HumanEntity player,
            Function<T, W> craftProcess,
            BiFunction<W, Location, MultiCraftingOperation> callBack,
            Consumer<T> failCallback) {
    ...
    public Function<CraftingInventory, Pair<MachineRecipe, IntFunction<ItemGreedyConsumer[]>>> CRAFT_PROCESSOR =
            (inventory1) -> {
                ...
                return CraftUtils.matchNextShapedRecipe(
                        inv,
                        new int[] {1, 2, 3, 4, 5, 6, 7, 8, 9},
                        SmithInterfaceProcessor.getInterfacedCrafttableRecipes(),
                        64,
                        false,
                        Settings.SEQUNTIAL,
    ...
    public Function<SmithingInventory, Supplier<MultiCraftingOperation>> SMITH_PROCESSOR = (smithingInventory -> {
        ...
        Supplier<MultiCraftingOperation> matchedLogic = null;
        for (var logic :
                logics.computeIfAbsent(SmithInterfaceProcessor.SmithOperation.BOTH, (b) -> new LinkedHashSet<>())) {
                ...
                (result, loc) -> {
                    ...
                    if (item != null && item.isDisabled()) {
                        ...
                        return new MultiCraftingOperation(
                                result.getSecondValue().apply(64),
                                result.getFirstValue().getTicks());
                    }
                },
                (inventory1) -> {

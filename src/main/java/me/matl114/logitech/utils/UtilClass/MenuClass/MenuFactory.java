...
public abstract class MenuFactory {
    ...
    public MenuFactory makeFinal() {
        ...
        if (inventory.size() != handlers.size()) {
            throw new RuntimeException(
                    "An internal error occurs while creating Menu ... inventory size doesn't match handler size : size "
                            + inventory.size() + " and size " + handlers.size());
        }
    ...
    public MenuFactory setGuideModHistory(PlayerHistoryRecord<CustomMenu> guideBuilder) {
        if (guideBuilder != null) this.guideBuilder = guideBuilder;
        return this;
    }

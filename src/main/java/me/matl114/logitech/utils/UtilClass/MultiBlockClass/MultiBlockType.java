...
public abstract class MultiBlockType implements AbstractMultiBlockType {
    ...
    public String getRequirementPartId(int index) {
        return REQUIREMENT_IDS[index];
    }

    public BlockVector getRequirementPart(int index) {
        return REQUIREMENT_LOC[index].clone();
    }
    ...
    public AbstractMultiBlock genMultiBlockFrom(
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (!this.getSchemaPartId(i).equals(MultiBlockService.safeGetPartId(partloc))) {
                ...
                final int a = i;
                errorOut.out(() -> {
                    return "&c位于%s的多方块部件并不是有效的部件,需要%s"
                            .formatted(DataCache.locationToDisplayString(partloc), this.getSchemaPartId(a));
                });
                return null;
            } else {
        ...
        if (!hasPrevRecord) {
            ...
            for (int i = 0; i < len2; i++) {
                ...
                if (!REQUIREMENT_IDS[i].equals(MultiBlockService.safeGetPartId(reqLoc))) {
                    ...
                    int a = i;
                    errorOut.out(() -> {
                        return "&c位于%s的方块并不满足构建所需要的额外条件".formatted(DataCache.locationToDisplayString(reqLoc));
                    });
                    return null;
                }
            }

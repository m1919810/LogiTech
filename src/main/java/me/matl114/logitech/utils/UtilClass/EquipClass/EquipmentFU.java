...
public abstract class EquipmentFU {
    ...
    private static final EnumMap<EquipmentSlot, HashMap<NamespacedKey, EquipmentFU>> registry =
    ...
    public EquipmentFU(NamespacedKey key, FURarity rarity, EquipmentSlot... slot) {
        ...
        if (this.slot.contains(EquipmentSlot.HAND) || this.slot.contains(EquipmentSlot.OFF_HAND)) {
            ...
            this.slot.forEach(slotType -> {
                switch (slotType) {
                    case HEAD:
                        materials.addAll(WorldUtils.HELMET_MATERIALS);
                        break;
                    case CHEST:
                        materials.addAll(WorldUtils.CHESTPLATE_MATERIALS);
                        break;
                    case LEGS:
                        materials.addAll(WorldUtils.LEGGINGS_MATERIALS);
                        break;
                    case FEET:
                        materials.addAll(WorldUtils.BOOTS_MATERIALS);
                        break;
                    default:
                        break;
                }
    ...
    public final <T extends Event> EquipmentFU registerElseEventHandler(
            Class<T> eventIdentifier, FUHandler<T> handler) {
        elseEventHandlers
                .computeIfAbsent(eventIdentifier, (e) -> new HashSet<>())
                .add(handler);
     ...
     * @param fuDataField
     */
    public void onEquipLevelChange(
            @Nonnull ItemStack equipment,
            @Nonnull ItemMeta editingMeta,
            @Nonnull PersistentDataContainer fuDataField,
            int newLevel) {

...
public abstract class CustomItemGroup extends FlexItemGroup {
     ...
     * @param subGroup
     */
    //    public CustomItemGroup(NamespacedKey key, String title, ItemStack item, int size,int inventorylen,
    // List<ItemGroup> subGroup){
    //        super(key,item);
    //
     ...
     * @param pages
     */
    protected abstract void addGuideRelated(
    ...
    protected int[] previewGroupSlot() {
        ...
        for (int j = 0; j < groupSize; j++) {
            // 非空,会一直寻找直到找到一个空的位置
            while (!(menuFactory.getInventory(i) == null
                    && (menuFactory.getHandler(i) == null
                            || menuFactory.getHandler(i) == ChestMenuUtils.getEmptyClickHandler()))) {
                ++i;
            }

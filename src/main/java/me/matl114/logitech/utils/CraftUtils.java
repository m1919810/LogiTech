...
public class CraftUtils {
        ...
        for (int i : slots) {
            ...
            if (stack == null || stack.getType().isAir() || stack.getAmount() >= stack.getMaxStackSize()) {
                ...
                for (int j = 0; j < len; ++j) {
                    ...
                    if (matcher != null && useSimilar
                            ? matcher.getItem().isSimilar(stack)
                            : matchItemCore(matcher, counter, true)) {
    ...
    public static ItemConsumer[] countOneOutput(
        ...
        int cnt = recipeInput.length;
        ItemConsumer[] result = new ItemConsumer[cnt];
        DynamicArray<ItemPusher> slotCounters =
                new DynamicArray<>(ItemPusher[]::new, len2, pusher.getMenuInstance(Settings.OUTPUT, inv, output));
        for (int i = 0; i < cnt; ++i) {
            result[i] = getConsumer(recipeInput[i]);
            for (int j = 0; j < len2; ++j) {
    ...
    public static Pair<ItemConsumer[], ItemConsumer[]> countOneRecipe(
        ...
        DynamicArray<ItemPusher> inputs =
                new DynamicArray<>(ItemPusher[]::new, len, pusher.getMenuInstance(Settings.INPUT, inv, input));
    ...
    public static ItemGreedyConsumer[] countMultiOutput(
        ...
        int len2 = output.length;
        DynamicArray<ItemPusher> outputCounters =
                new DynamicArray<>(ItemPusher[]::new, len2, pusher.getMenuInstance(Settings.OUTPUT, inv, output));
    ...
    public static boolean forcePush(
        ...
        DynamicArray<ItemPusher> slotCounters2 = new DynamicArray<>(
                ItemPusher[]::new, slots.length, pusher.getMenuInstance(Settings.OUTPUT, inv, slots));
    ...
    public static Pair<MachineRecipe, ItemConsumer[]> findNextRecipe(
            BlockMenu inv, int[] slots, int[] outs, List<MachineRecipe> recipes, boolean useHistory, Settings order) {
    ...
    public static Pair<MachineRecipe, ItemConsumer[]> findNextRecipe(
        ...
        final DynamicArray<ItemPusher> slotCounter =
                new DynamicArray<>(ItemPusher[]::new, len, pusher.getMenuInstance(Settings.INPUT, inv, slots));
    ...
    public static MachineRecipe matchNextRecipe(
            BlockMenu inv, int[] slots, List<MachineRecipe> recipes, boolean useHistory, Settings order) {
    ...
    public static int matchNextRecipe(
            BlockMenu inv,
            int[] slots,
            List<MachineRecipe> recipes,
            int historyIndex,
    ...
    public static Pair<MachineRecipe, ItemGreedyConsumer[]> matchNextMultiRecipe(
        ...
        DynamicArray<ItemPusher> slotCounter = new DynamicArray<>(
                ItemPusher[]::new, slotNotNull.size(), pusher.getMenuInstance(Settings.INPUT, inv, slotNotNull));
        int recipeAmount = recipes.size();
        if (recipeAmount <= 0) {
            return null;
        }
    ...
    public static Pair<MachineRecipe, ItemGreedyConsumer[]> countShapedRecipe(
            BlockMenu inv,
    ...
    public static Pair<MachineRecipe, ItemGreedyConsumer[]> matchShapedRecipe(
            BlockMenu inv,
            int[] inputs,
            int[] outputs,
            List<MachineRecipe> recipes,
    ...
    public static MachineRecipe findNextShapedRecipe(
            BlockMenu inv,
            int[] inputs,
            int[] outputs,
            List<MachineRecipe> recipes,
    ...
    public static boolean matchItemCore(ItemCounter counter1, ItemCounter counter2, boolean strictCheck) {
        ...
        //        if (hasAttributeOne) {
        //            if (!hasAttributeTwo ||
        // !Objects.equals(meta1.getAttributeModifiers(),meta2.getAttributeModifiers())) {
        //                return false;
        //            }
    ...
    //        return
    // displayNameAccess//displayNameAccess.compareFieldOrDefault(meta1,meta2,()->Objects.equals(meta1.getDisplayName(),meta2.getDisplayName()));
    //    }

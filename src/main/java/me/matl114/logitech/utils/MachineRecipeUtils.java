...
public class MachineRecipeUtils {
    ...
    public static ItemStack[] i(ItemStack[] stacklist) {
        ...
        for (int i = 0; i < len; i++) {
            if (stacklist[i] != null
                    && (((stacklist[i] instanceof AbstractItemStack))
                            || !stacklist[i].getType().isAir())) {
    ...
    public static StackMachineRecipe stackFromMultiBlock(ItemStack[] input, MultiBlockMachine machine, int ticks) {
        ...
        return getNoComsumed(
                new StackMachineRecipe(ticks, stackIn(input), new ItemStack[] {recipeOutput}), StackMachineRecipe::new);
    }
    ...
    public static StackMachineRecipe stackFromRecipe(SlimefunItem item, int time) {
        ...
        //        }
        return getNoComsumed(
                new StackMachineRecipe(time, stackIn(item.getRecipe()), new ItemStack[] {item.getRecipeOutput()}),
                StackMachineRecipe::new);
    }
    /**
    ...
    public static StackMachineRecipe shapeFromMultiBlock(ItemStack[] input, MultiBlockMachine machine, int ticks) {
        ...
        return getNoComsumed(
                new StackMachineRecipe(ticks, i(input), new ItemStack[] {recipeOutput}), StackMachineRecipe::new);
    }
    ...
    public static MGeneratorRecipe mgFromMachine(MachineRecipe recipe) {
        if (recipe instanceof MGeneratorRecipe recipe1) {
            ...
            return new MGeneratorRecipe(
                    recipe.getTicks(), stackIn(makeValidMGInput(recipe.getInput())), stackIn(recipe.getOutput()));
        }
    }
    ...
    public static MGeneratorRecipe tryGenerateMGFromMachine(MachineRecipe recipe) {
        ...
        if (input.length != 0 && output.length != 0) {
            if (input.length == 1
                    && output.length == 1
                    && CraftUtils.matchItemStack(output[0], input[0], true)
                    && input[0].getAmount() >= output[0].getAmount()) {
                return mgFrom(recipe1.getTicks(), input, output);
            }

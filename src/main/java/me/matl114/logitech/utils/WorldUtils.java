...
public class WorldUtils {
    ...
    public static final BlockDataController CONTROLLER =
    ...
    public static EnumSet<Material> BLOCKTYPE_WITH_NONNULL_TICKER = EnumSet.copyOf(new HashSet<Material>() {
        {
    ...
    // till 1.20.4
    public static EnumSet<Material> BLOCK_MUST_WATERLOGGED =
            EnumSet.of(Material.SEAGRASS, Material.TALL_SEAGRASS, Material.KELP_PLANT, Material.KELP);
    public static EnumSet<Material> BLOCK_WITH_RANDOMTICK = EnumSet.copyOf(new HashSet<Material>() {
        {
    ...
    public static EnumSet<Material> BLOCK_WITH_MULTI_BLOCKSTATE = EnumSet.copyOf(new HashSet<Material>() {
        {
    ...
    public static final EnumSet<Material> HELMET_MATERIALS = EnumSet.noneOf(Material.class);
    public static final EnumSet<Material> CHESTPLATE_MATERIALS = EnumSet.noneOf(Material.class);
    public static final EnumSet<Material> LEGGINGS_MATERIALS = EnumSet.noneOf(Material.class);
    public static final EnumSet<Material> BOOTS_MATERIALS = EnumSet.noneOf(Material.class);
    ...
    @Nullable public static Material getBlock(Material material) {
    ...
    static {
        try {
            ...
            var BlockPositionField = ReflectUtils.getDeclaredFieldsRecursively(CraftBlockStateClass, "position")
                    .getFirstValue();
            BlockPositionField.setAccessible(true);
            blockPositionFieldAccess = FieldAccess.of(BlockPositionField);
            var WorldField = ReflectUtils.getDeclaredFieldsRecursively(CraftBlockStateClass, "world")
                    .getFirstValue();
            WorldField.setAccessible(true);
            worldFieldAccess = FieldAccess.of(WorldField);
            var WeakWorldField = ReflectUtils.getDeclaredFieldsRecursively(CraftBlockStateClass, "weakWorld")
                    .getFirstValue();
    ...
    public static void removeSlimefunBlock(Location loc, Player player, boolean force) {
        if (force) {
            ...
            BlockBreakEvent breakEvent = new BlockBreakEvent(loc.getBlock(), player);
            // set drop to false
    ...
    public static boolean createSlimefunBlock(
            Location loc, Player player, SlimefunItem item, Material material, boolean force, Runnable callback) {
    ...
    public static boolean createSlimefunBlockSync(
            Location loc, Player player, SlimefunItem item, Material material, boolean force, Runnable callback) {
        if (abort) {
            return false;
        }
    ...
    public static boolean hasPermission(
            Player player, @Nonnull Location location, @Nonnull Interaction... interactions) {
        if (player == null) return true;
        for (Interaction interaction : interactions) {
            if (!Slimefun.getProtectionManager().hasPermission(player, location, interaction)) {
                return false;
    ...
    public static int executeOnSameEntity(Entity entity, Consumer<Entity> execution) {
        ...
        return entity.getLocation()
                .getChunk()
                .getWorld()
                .getNearbyEntities(entity.getLocation(), 0.5, 0.5, 0.5, (entity1 -> {
    ...
    public static HashSet<Entity> getEntityInDistance(Location location, double distance, Predicate<Entity> predicate) {
        return new HashSet<Entity>(
                location.getWorld().getNearbyEntities(location, 2 * distance, 2 * distance, 2 * distance, (e) -> {
                    return e.getLocation().distance(location) <= distance && predicate.test(e);
                }));
    }

    public static Pair<Integer, Location> rayTraceLocation(
            LivingEntity entity, double period, double maxLimitedDistance, Predicate<Location> execution) {
        if (entity == null) return null;
        return rayTraceLocation(
                entity.getLocation().getDirection(), entity.getEyeLocation(), period, maxLimitedDistance, execution);
    }
    ...
    public static Pair<Integer, Location> rayTraceLocation(
            ...
            double period,
            double maxLimitedDistance,
            Predicate<Location> execution) {
    ...
    public static boolean isLightPassableBlock(Block block) {
        ...
        if (material.isAir()
                ...
                || WATER_VARIENT.contains(material)
    ...
    public static boolean breakVanillaBlockByPlayer(
            Block block, Player player, boolean hasCheckedProtection, boolean withDrop) {
    ...
    public static boolean moveVanillaBlockByPlayer(
            Block block1,
            Block block2,
            Player player,
            boolean checkFromPerms,
            boolean checkToPerms,
            boolean hasCheckedProtection,
            boolean applyPhysics) {
        if (hasCheckedProtection
                || ((!checkFromPerms || WorldUtils.hasPermission(player, block1, Interaction.BREAK_BLOCK))
                        && (!checkToPerms || WorldUtils.hasPermission(player, block2, Interaction.PLACE_BLOCK)))) {
            if (checkFromPerms) {
    ...
    public static boolean testVanillaBlockBreakPermission(Block block, Player player, boolean hasCheckedProtection) {
        if (hasCheckedProtection
                || (WorldUtils.hasPermission(player, block, Interaction.BREAK_BLOCK, Interaction.PLACE_BLOCK))) {
            BlockBreakEvent event = new BlockBreakEvent(block, player);
            event.setDropItems(false);
            event.setExpToDrop(0);
            Bukkit.getPluginManager().callEvent(event);

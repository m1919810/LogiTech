...
public class NMSUtils {
    ...
    public static final MethodAccess<?> getBlockEntityTypeMethodAccess = new MethodAccess((obj) -> {
        List<Class> cls = ReflectUtils.getAllSuperClassRecursively(obj.getClass()).stream()
                .filter(s -> "TileEntity".equals(s.getSimpleName()))
                .toList();
        Class clazz = cls.get(cls.size() - 1);
        // errorOut.accept(clazz);
        tileEntityClass = clazz;
        var re = Arrays.stream(clazz.getDeclaredMethods())
                .filter(m -> m.getParameterTypes().length == 0)
                .filter(m -> m.getReturnType() != boolean.class)
                .filter(s -> s.getReturnType().getSimpleName().endsWith("Types"))
                .toList();
        // errorOut.accept(re);
    ...
    public static final MethodAccess<?> getTickerMethodAccess = new MethodAccess((obj) -> {
        ...
        Method a = Arrays.stream(methods)
    ...
    public static final MethodAccess<?> tickMethodAccess = new MethodAccess((obj) -> {
        ...
        var re = Arrays.stream(a)
                .filter((m) -> m.getParameterTypes().length == 4)
                .filter(m -> "tick".equals(m.getName()))
                .toList();
        return re.isEmpty() ? null : re.get(0);
    });
    public static final MethodAccess<?> randomTickAccess = new MethodAccess((o -> {
                ...
                return methods.stream()
                        ...
                        .filter(m -> m.getParameterTypes()[2].getName().endsWith("RandomSource"))
                        .findFirst()
                        .get();
            }))
            .printError(true);
    public static final FieldAccess randomSourceAccess = new FieldAccess((o) -> {
        ...
        var a = fields.stream()
                ...
                .filter(m -> m.getType().getName().endsWith("RandomSource"))
                .toList();
        // Debug.logger(a);
        return a.get(0);
    });

...
public class MenuUtils {
    ...
    public static final ItemStack PROCESSOR_SPACE =
            new CustomItemStack(Material.RED_STAINED_GLASS_PANE, "&6进程完成", "&c空间不足");
    ...
    public static ItemStack getPreviousButton(int page, int pages) {
        return pages != 1 && page != 1
                ? new CustomItemStack(PREV_BUTTON_ACTIVE, (meta) -> {
    ...
    public static ItemStack getNextButton(int page, int pages) {
        return pages != 1 && page != pages
                ? new CustomItemStack(NEXT_BUTTON_ACTIVE, (meta) -> {
                    ChatColor var10001 = ChatColor.WHITE;
    ...
    private static final int[] RECIPE_DISPLAY_INDEX2SLOT =
    ...
    public static final MenuPreset RECIPE_MENU_3X3 = new MenuPreset(27, 0, 0).addItem(BACK_BUTTON, 1);
    public static final MenuPreset RECIPE_MENU_3X3_WITH_DISPLAY = new MenuPreset(54, 36, 0).addItem(BACK_BUTTON, 1);
    // not used yet need
    public static final MenuPreset RECIPE_MENU_3x3_PLUS =
            new MenuPreset(54, 0, 0).addItem(ChestMenuUtils.getBackground(), 27, 29, 30, 31, 32, 33, 35);
    // 6x6大配方的显示
    ...
    public static final MenuPreset RECIPE_DISPLAY_SINGLE = new MenuPreset(36, 18, 0).addItem(BACK_BUTTON, 1);
    ...
    public static final int[] RECIPESLOT_6x6 = new int[] {
        1, 2, 3, 4, 5, 6, 10, 11, 12, 13, 14, 15, 19, 20, 21, 22, 23, 24, 28, 29, 30, 31, 32, 33, 37, 38, 39, 40, 41,
        42, 46, 47, 48, 49, 50, 51
    };
    public static final int[] RECIPEOUTPUT_6X6 = new int[] {35, 44};

    public interface RecipeMenuConstructor {
        public MenuFactory construct(
                ItemStack icon,
                MachineRecipe recipe,
                CustomMenuHandler backhandler,
                SlimefunItem optionalItem,
                PlayerHistoryRecord<CustomMenu> history);

        default MenuFactory construct(
                ItemStack icon,
                MachineRecipe recipe,
                CustomMenuHandler backhandler,
                PlayerHistoryRecord<CustomMenu> history) {
    ...
    public static MenuFactory createMRecipeListDisplay(
            ItemStack machine, List<MachineRecipe> machineRecipes, @Nullable CustomMenuHandler backHandler) {
    ...
    public static MenuFactory createMRecipeListDisplay(
            ...
            @Nullable CustomMenuHandler backHandler,
            PlayerHistoryRecord<CustomMenu> history) {
    ...
    public static MenuFactory createMRecipeListDisplay(
            ...
            List<MachineRecipe> machineRecipes,
            @Nullable CustomMenuHandler backHandler,
            RecipeMenuConstructor constructor) {
    ...
    public static MenuFactory createMRecipeListDisplay(
        ...
        for (int i = 0; i < RecipeSize; i++) {
            ...
            a.addHandler(i, (cm) -> (player, i1, itemStack, clickAction) -> {
                constructor
                        .construct(machine, recipe, cm.getBackToThisHandler(pageNow), history)
                        .build()
                        .open(player);
                return false;
            });
    ...
    public static MenuFactory createItemListDisplay(
            RecipeType type, List<SlimefunItem> items, @Nullable CustomMenuHandler backHandler) {
    ...
    public static MenuFactory createItemListDisplay(
        ...
        MenuFactory a = new MenuFactory(
                SIMPLE_MENU, AddUtils.color("类型:" + type.getKey().toString()), pageNum) {
            public void init() {
                setDefaultNPSlots();
            }
        }.addOverrides(7, displayMachine);
        ...
        for (int i = 0; i < RecipeSize; i++) {
            ...
            a.addHandler(i, (cm) -> (player, i1, itemStack, clickAction) -> {
                constructor
                        .construct(
                                displayMachine,
                                new MachineRecipe(0, item.getRecipe(), new ItemStack[] {item.getRecipeOutput()}),
                                cm.getBackToThisHandler(pageNow),
                                item,
                                history)
                        .build()
                        .open(player);
            ...
            List<String> namelist = new ArrayList<>() {
                {
                    ...
                    for (int i = 0; i < len; i++) {
                        add(AddUtils.concat(
                                ...
                                " 概率: ",
                                AddUtils.getPercentFormat(wlist.get(i))));
                    }
                }
            };
            ...
            List<String> namelist = new ArrayList<>() {
                {
                    ...
                    for (int i = 0; i < len; i++) {
                        add(AddUtils.concat(
                                "&f", CraftUtils.getItemName(list.get(i)), " ", getAmountDisplay(list.get(i))));
                    }
                }
            };
            ItemStack sample = list.get(0);
    ...
    public static MenuFactory createMRecipeDisplay(
            ItemStack machine, MachineRecipe recipe, @Nullable CustomMenuHandler backHandler) {
        return createMRecipeDisplay(machine, recipe, backHandler, null, null);
    }

    public static MenuFactory createMRecipeDisplay(
            ItemStack machine, MachineRecipe recipe, @Nullable CustomMenuHandler backHandler, SlimefunItem optional) {
    ...
    public static MenuFactory createMRecipeDisplay(
            ...
            MachineRecipe recipe,
            @Nullable CustomMenuHandler backHandler,
            @Nullable SlimefunItem optionalItem,
            @Nullable PlayerHistoryRecord<CustomMenu> history) {
        ...
        if (inputlen <= 9) {
            ...
            getInfo:
            {
                if (optionalItem instanceof RecipeDisplayItem recipeDisplay) {
                    ...
                    if (!displayedDown.isEmpty()) {
                        a =
                                new MenuFactory(
                                        RECIPE_MENU_3X3_WITH_DISPLAY,
                                        AddUtils.colorful("配方展示"),
                                        ((displayedDown.size() + 17) / 18)) {
                                    @Override
                                    public void init() {}
                                };
                        break getInfo;
            ...
            if (machine != null && !machine.getType().isAir()) {
                if (recipe.getTicks() >= 0)
                    a.addOverrides(
                            RECIPETYPESLOT_3X3, AddUtils.addLore(machine, "", "&a在此处耗时" + recipe.getTicks() + "&at制作"));
                else a.addOverrides(RECIPETYPESLOT_3X3, AddUtils.addLore(machine, "", "&a在此处制作"));
            }
            for (int i = 0; i < input.length; ++i) {
                ...
                if (sfitem != null) {
                    a.addOverrides(RECIPESLOT_3X3[i], (cm) -> ((player, i1, itemStack, clickAction) -> {
                        createItemRecipeDisplay(sfitem, cm.getBackToThisHandler(1), history)
                                .build()
                                .open(player);
                        return false;
                    }));
            ...
            if (!displayedDown.isEmpty()) {
                ...
                for (int i = 0; i < size; ++i) {
                    ...
                    int indexI = i % 18;
                    a.addInventory(
                            18 * pageI + (RECIPE_DISPLAY_INDEX2SLOT[indexI] - 36),
                            displayedDown.get(i),
                            (cm1) -> ((player1, i1, itemStack1, clickAction1) -> {
                                SlimefunItem item = SlimefunItem.getByItem(itemStack1);
                                if (item != null) {
                                    createItemRecipeDisplay(item, cm1.getBackToThisHandler(1), history)
                                            .build()
                                            .open(player1);
                                }
                                return false;
            ...
            if (recipe.getTicks() >= 0)
                a.addInventory(
                        RECIPETYPESLOT_6X6, AddUtils.addLore(machine, "", "&a在该机器中耗时" + recipe.getTicks() + "&at制作"));
            else a.addInventory(RECIPETYPESLOT_6X6, AddUtils.addLore(machine, "", "&a在该机器中制作"));
            ...
            if (optionalItem instanceof RecipeDisplayItem recipeDisplay) {
                a.addOverrides(
                        53,
                        AddUtils.addGlow(new CleanItemStack(Material.KNOWLEDGE_BOOK, "&a点击查看物品展示的配方", "&7以及物品额外说明")),
                        (cm) -> ((player, i, itemStack, clickAction) -> {
                            ...
                            var recipeMenu =
                                    new MenuFactory(
                                            RECIPE_DISPLAY_SINGLE,
                                            CraftUtils.getItemName(optionalItem.getItem()),
                                            (displayedDown.size() + 17) / 18) {
                                        @Override
                                        public void init() {}
                                    };
                            ...
                            for (int index = 0; index < size; ++index) {
                                int pageI = index / 18;
                                int indexI = index % 18;
                                recipeMenu.addInventory(
                                        18 * pageI + (RECIPE_DISPLAY_INDEX2SLOT[indexI] - 36),
                                        displayedDown.get(index),
                                        (cm1) -> ((player1, i1, itemStack1, clickAction1) -> {
                                            SlimefunItem item = SlimefunItem.getByItem(itemStack1);
                                            if (item != null) {
                                                createItemRecipeDisplay(item, cm1.getBackToThisHandler(1), history)
    ...
    public static MenuFactory createItemRecipeDisplay(
            SlimefunItem item, @Nullable CustomMenuHandler backHandler, PlayerHistoryRecord<CustomMenu> history) {
        return createMRecipeDisplay(
                item.getRecipeType().toItem(),
                new MachineRecipe(-1, item.getRecipe(), new ItemStack[] {item.getRecipeOutput()}),
                backHandler,
                item,
                history);
    }
    ...
    public static MenuFactory createRecipeTypeDisplay(
            List<RecipeType> recipeTypes, CustomMenuHandler backHandler, PlayerHistoryRecord<CustomMenu> history) {
        ...
        for (RecipeType entry : recipeTypes) {
            ...
            if (recipe == null || recipe.isEmpty()) {
                ...
                a.addHandler(i, (customMenuHandler) -> ((player, i1, itemStack, clickAction) -> {
                    createItemListDisplay(entry, recipe, customMenuHandler.getBackToThisHandler(currentPage))
                            .build()
                            .open(player);
                    return false;
                }));
    ...
    public static MenuFactory createMachineListDisplay(
            List<SlimefunItem> machineTypes, CustomMenuHandler backHandler, PlayerHistoryRecord<CustomMenu> history) {
        ...
        for (SlimefunItem entry : machineTypes) {
            ...
            if (recipe == null) {
                ...
                a.addHandler(i, (cm) -> (player, i1, itemStack, clickAction) -> {
                    createMRecipeListDisplay(entry.getItem(), machineRecipes, cm.getBackToThisHandler(currentPage))
                            .build()
                            .open(player);
                    return false;
                });

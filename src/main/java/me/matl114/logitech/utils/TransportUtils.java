...
public class TransportUtils {
    ...
    public static ItemPusher[] cacheTransportation(
            BlockMenu cacheInv, ItemPusher[] cachelst, BlockMenu menu, int[] slots, Settings mod) {
     ...
     * @return
     */
    public static ItemPusher[] transportItemFromCache(
            BlockMenu cacheInv, ItemPusher[] cachelst, BlockMenu menu, int[] slots, ItemPusherProvider provider) {
     ...
     * @return
     */
    public static ItemPusher[] transportItemToCache(
            BlockMenu cacheInv, ItemPusher[] cachelst, BlockMenu menu, int[] slots, ItemPusherProvider provider) {
        int len = slots.length;
        ItemPusher tarCounter2;
        ItemPusher[] tarCounter = new ItemPusher[len];
    ...
    private static int[] getSlotAccess(
            BlockMenu from, int configCode, boolean isFrom, boolean notDefaultPriority, ItemStack item) {
        if (isFrom) {
            ...
            if (CargoConfigs.FROM_REVERSED.getConfig(configCode)) {
                int[] fromSlot2 = notDefaultPriority
                        ? from.getPreset().getSlotsAccessedByItemTransport(from, ItemTransportFlow.WITHDRAW, item)
                        : from.getPreset().getSlotsAccessedByItemTransport(from, ItemTransportFlow.INSERT, item);
    ...
    public static void transportItem(
            BlockMenu from,
            BlockMenu to,
            int configCode,
            boolean smart,
            HashSet<ItemStack> bwlist,
            ItemPusherProvider provider) {
    ...
    public static void transportItem(
            ...
            HashSet<ItemStack> bwList,
            ItemPusherProvider provider) {
        transportItemEnsureLock(
                from, fromSlot, to, toSlot, configCode, smart, bwList, ItemTransportFlow.INSERT, provider);
    }
    ...
    //            = new ObjectLockFactory<Location>(Location.class,Location::clone)
    // .init(MyAddon.getInstance()).setupRefreshTask(10*20*60);
    public static void setup() {
        try {
            Object lockFactory = MethodAccess.ofName(Slimefun.class, "getCargoLockFactory")
                    ...
                    .initWithNull()
                    .invoke(null);
            transportationLockFactory = ProxyBuilder.buildMatlibAdaptorOf(
                    LockFactory.class,
                    lockFactory,
    ...
    private static void transportItemEnsureLock(
            BlockMenu from,
            int[] fromSlot,
            BlockMenu to,
            int[] toSlot,
            ...
            boolean smart,
            HashSet<ItemStack> bwList,
            ItemTransportFlow mainDestinationFlow,
            ItemPusherProvider provider) {
        ...
        transportationLockFactory.ensureLock(
                () -> {
                    ...
                    if (issymm) {
                        transportItemSymm(
                                from, fromSlot, to, toSlot, isnull, islazy, iswtlist, bwList, limit, provider);
                    } else {
                        if (!smart) {
                            transportItemGreedy_2(
                                    from, fromSlot, to, toSlot, isnull, islazy, iswtlist, bwList, limit, provider);
                        } else {
                            ...
                            if (multiDestinationSlot) {
                                transportItemSmart_2(
                                        ...
                                        provider,
                                        mainDestinationFlow,
                                        mainDestinationFlow == ItemTransportFlow.INSERT
    ...
    private static void transportItemGreedy_2(
            BlockMenu from,
            int[] fromSlot,
            BlockMenu to,
            int[] toSlot,
        ...
        DynamicArray<ItemPusher> toCache =
                new DynamicArray<>(ItemPusher[]::new, 54, provider.getMenuInstance(Settings.OUTPUT, to));
     ...
     * @param provider
     */
    private static void transportItemSmart_2(
            BlockMenu from,
            int[] fromSlot,
            BlockMenu to,
            int[] toSlot,
    ...
    @Deprecated
    private static void transportItemGreedy_3_workspace(
            BlockMenu from,
            int[] fromSlot,
            BlockMenu to,
            int[] toSlot,
        ...
        // 我那么大一个输出槽呢？
        if (indexLen <= 0) return;
        DynamicArray<ItemPusher> toCache = new DynamicArray<>(
                ItemPusher[]::new, indexLen, provider.getMenuInstance(Settings.OUTPUT, to, toCacheIndex));
    ...
    private static void transportItemSymm(
            BlockMenu from,
            int[] fromSlot,
            BlockMenu to,
            int[] toSlot,

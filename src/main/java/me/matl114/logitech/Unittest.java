...
public class Unittest {
    ...
    // 修复刷物
    // 自动跑图机 + 随机进度条
    // 自动跑图机模拟真实的 加入风杖 鞘翅和多功能工具 加入食物消耗 加入种子
    // 种子机制 小于真实种子 概率25% 大于 75% 等于 100% 不输入 50%
    //fixme some recipe seems to return glass-bottle when no bottle-like item in recipe -纳米工作台
    //TODO logisuit
    //todo logisuit 叫功能单元吧
    // todo 切换鞘翅功能 电力格挡(直接取消和修改伤害值) 但是不阻挡同附属伤害()
    //todo 防凋零（呼吸过滤装置） 防辐射 等等 多元效果
    //修改当遇到其他充电桩时粒子效果数量
    //物流管道
    //TODO 限制存储单元?这啥
    //TODO 白名单机器接口
    ...
    //todo 纠缠推送抓取器
    //todo 配置器增加配置物
    //修复配方空桶合并失败的问题 TODO 增加通用随机物品解析？ 这是啥
    ...
    //todo 使用投影 做棋子  农耕那种
    //todo 增加货运卡片右键设置
    //todo 投影学习交互实体，点击显示信息
    //增加量子垃圾桶
    //fix the error in AUTO_BUILD
    //ATTR_OP 增加反向选择
    //加堆叠 资源合成 台锯
    //增加了全部匹配物品，
    //增加特殊设置接口
    ...
    //todo
    //a possibility to handle vanilla block in MultiBlock structures ,maybe we need more listeners and fix the getStatus and getUUID method in MultiBlockService
    
    //todo 优化投影体系
    //修复堆叠机器久置后内部机器变砖的问题->sf issue!
    //做测试道具-多方块自动构建机
    //todo 增加物品效果管理器
    //todo 1 手中握着物品 事件触发

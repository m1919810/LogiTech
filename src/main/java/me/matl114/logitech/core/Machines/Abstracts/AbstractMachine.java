 ...
 */
public abstract class AbstractMachine extends CustomSlimefunItem
        implements Ticking, MenuBlock, EnergyNetComponent, NotHopperable {
    ...
    public void addInfo(ItemStack stack) {
        if (this.energyConsumption != 0) {
            stack.setItemMeta(AddUtils.machineInfoAdd(stack, this.energybuffer, this.energyConsumption)
                    .getItemMeta());
        } else if (this.energyNetComponent == EnergyNetComponentType.CAPACITOR) {
            stack.setItemMeta(
                    AddUtils.capacitorInfoAdd(stack, this.energybuffer).getItemMeta());
        }
    }
    ...
    public boolean conditionHandle(Block b, BlockMenu menu) {
        if (menu != null) return this.handleProcess.test(menu.getLocation());
        else return this.handleProcess.test(b.getLocation());
    }
    ...
    protected void progressorCost(Block b, BlockMenu menu) {
        if (menu != null) this.handlerProcessCost.accept(menu.getLocation());
        else this.handlerProcessCost.accept(b.getLocation());
    }

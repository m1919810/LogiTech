...
public abstract class AbstractEnergyCollector extends AbstractEnergyMachine
        implements EnergyProvider, MenuTogglableBlock {
    ...
    protected final ItemStack LAZY_ITEM_OFF = new CustomItemStack(
            Material.RED_STAINED_GLASS_PANE, "&6点击切换懒惰模式", "&7当前状态: &c关闭", "&7当启用懒惰模式时,只有自身电力不满时", "&7才会尝试收集发电机电力");
    protected final ItemStack LAZY_ITEM_ON = new CustomItemStack(
            Material.GREEN_STAINED_GLASS_PANE, "&6点击切换懒惰模式", "&7当前状态: &a开启", "&7当启用懒惰模式时,只有自身电力不满时", "&7才会尝试收集发电机电力");

    public AbstractEnergyCollector(
    ...
    protected ConcurrentHashMap<Location, SlimefunBlockData> ERROR_MACHINES =
            new ConcurrentHashMap<Location, SlimefunBlockData>();
    ...
    public void tick(Block b, BlockMenu menu, SlimefunBlockData data, int ticker) {
                    ...
                    CompletableFuture<Boolean> future1 = CompletableFuture.supplyAsync(() -> {
                            ...
                            .exceptionally(ex -> {
                                ...
                                ERROR_MACHINES.compute(testLocation, (loc1, data1) -> {
                                    if (data1 != sf) {
                                        Debug.logger(
                                                "Error while Ticking Generator at:",
                                                DataCache.locationToDisplayString(loc1));
        ...
        if (menu.hasViewer()) {
            menu.replaceExistingItem(
                    getInfoSlot(), getInfoShow(charge.get(), energyProvider.get(), errorMachine.get()));
        }
        if (!gens.isEmpty()) {
            ...
            for (SlimefunBlockData sf : allDatas) {
                ...
                if (ec != null) {
                    ...
                    CompletableFuture<Boolean> future1 = CompletableFuture.supplyAsync(() -> {
                            ...
                            .exceptionally(ex -> {
                                errorMachine.getAndIncrement();
                                new ErrorReport<>(ex, testLocation, item);
                                return null;
                            });
                    gens.add(future1);
                    future1.thenApply((more) -> {

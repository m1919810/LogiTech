...
public class StackMGenerator extends MMGenerator implements MultiCraftType, ImportRecipes {
    protected final int[] BORDER = new int[] {6, 7, 8, 15, 16, 17};
    protected final int[] INPUT_SLOT = new int[] {0, 1, 2, 9, 10, 11};
    ...
    protected final ItemStack INFO_ITEM = new CustomItemStack(
            ...
            "&6机器会进行模拟,其中",
            "&7<并行处理数>=<机器数*工作效率>",
            "&7<耗电数>=min(<机器数量*单个机器耗电/工作效率>,<最大电容量>)",
            "&6有关高级机器和并行处理数的信息,请见粘液书<版本与说明>",
            "&6支持的机器可以在粘液书<通用机器类型大全>",
            "&6或者左边按钮查看",
            "&6机器支持的配方可以点击右侧按钮查看");
    ...
    protected final ItemStack MACHINEMENU_ICON =
            new CustomItemStack(Material.BLAST_FURNACE, "&b该机器支持的机器列表", "&6点击打开菜单");
    protected final ItemStack RECIPEMENU_ICON =
            new CustomItemStack(Material.KNOWLEDGE_BOOK, "&b当前模拟的机器配方列表", "&6点击打开菜单");

    protected ItemStack getInfoItem(int craftlimit, int energyCost, int charge, double efficiency, String name) {
        return new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE,
                "&a生成器信息",
                AddUtils.concat("&7当前模拟的生成器名称: ", (name)),
                "&7当前并行处理数: %-3d".formatted(craftlimit),
                "&7当前每刻耗电量: %sJ/t".formatted(AddUtils.formatDouble(energyCost)),
                "&7当前电量: %sJ".formatted(AddUtils.formatDouble(charge)),
    ...
    static {
        PostSetupTasks.addPostRegisterTask(() -> {
            getMachineList();
            MACHINE_LIST_MENU =
                    MenuUtils.createMachineListDisplay(getMachineList(), null).setBack(1);
        });
    }

    protected ItemPusherProvider MACHINE_PROVIDER = CraftUtils.getpusher;

    public StackMGenerator(
            ItemGroup itemGroup,
    ...
    public final void updateMenu(BlockMenu inv, Block block, Settings mod) {
        ...
        if (data == null) {
            Debug.logger("SF DATA LOST AT %s! PLEASE REPORT THIS LOG TO THE AUTHOR"
                    .formatted(DataCache.locationToString(inv.getLocation())));
            return;
        } else if (!data.isDataLoaded()) {
            Debug.logger("SF DATA NOT LOAD YET IN %s! PLEASE REPORT THIS LOG TO THE AUTHOR"
                    .formatted(DataCache.locationToString(inv.getLocation())));
        ...
        if (it != null) {
            ...
            if (sfitem != null) {
                ...
                if (sfitem == historyMachine) {
                    ...
                    if (CraftUtils.matchNextRecipe(
                                    inv,
                                    getInputSlots(),
                                    RecipeSupporter.MACHINE_RECIPELIST.get(historyMachine),
                                    // 不许用高级的 你是多有病才能在不消耗的槽里面塞存储
                                    true,
                                    Settings.SEQUNTIAL,
                                    CraftUtils.getpusher)
                            == null) {
                    ...
                    for (int i = 0; i < size; ++i) {
                        ...
                        if (sfitem == item || thisId.equals(item.getId())) {
                            ...
                            if (CraftUtils.matchNextRecipe(
                                            inv,
                                            getInputSlots(),
                                            RecipeSupporter.MACHINE_RECIPELIST.get(sfitem),
                                            true,
                                            Settings.SEQUNTIAL,
                                            CRAFT_PROVIDER)
                                    == null) {
    ...
    public List<MachineRecipe> getMachineRecipes(SlimefunBlockData data) {
        int index = MultiCraftType.getRecipeTypeIndex(data);
        if (index >= 0 && index < getListSize()) {
            List<MachineRecipe> lst =
                    RecipeSupporter.MACHINE_RECIPELIST.get(getMachineList().get(index));
        ...
        if (index >= 0 && index < getListSize()) { // 有效机器
            ...
            if (energy >= consumption) {
                if (hasViewer) {
                    ...
                    inv.replaceExistingItem(
                            ...
                            getInfoItem(
                                    ...
                                    energy,
                                    this.efficiency,
                                    CraftUtils.getItemName(this.MACHINE_PROVIDER
                                            .getPusher(Settings.INPUT, inv, this.MACHINE_SLOT)
                                            .getItem())));
                }
                ...
                if (tick <= 0) {
                    ...
                    List<MachineRecipe> lst = RecipeSupporter.MACHINE_RECIPELIST.get(
                            getMachineList().get(index));
                    int len = lst.size();
                    ...
                    if (index2 >= 0 && index2 < len) {
                        ...
                        if (tick == 0) {
                            ...
                            if (maxMultiple == 1) {
                                ...
                                CraftUtils.multiPushItems(
                                        nextP.getOutput(), inv, getOutputSlots(), maxMultiple, CRAFT_PROVIDER);
                            }
                        }
                ...
                if (hasViewer)
                    inv.replaceExistingItem(
                            ...
                            getInfoOffItem(
                                    ...
                                    consumption,
                                    energy,
                                    CraftUtils.getItemName(inv.getItemInSlot(MACHINE_SLOT))));
                if (tick != -1) {
                    db.setInt(2, -1);
                    if (hasViewer) inv.replaceExistingItem(this.PROCESSOR_SLOT, this.INFO_NULL);
                }
            }

...
public abstract class AbstractTimerRange extends AbstractMachine implements MenuTogglableBlock {
    ...
    public final void addLocation(Location loc) {
        if (DataCache.getMenu(loc) == null) {
            synchronized (lock) {
                TIMED_MACHINES.keySet().stream()
                        .filter(l -> l.getWorld() == loc.getWorld())
                        .filter(l -> inRange(loc, l, getRange()))
                        .filter(l -> DataCache.getSfItem(l) == AbstractTimerRange.this)
                        .forEach(l -> {
                            TIMED_MACHINES
                                    .computeIfAbsent(l, (le) -> new HashSet<>())
                                    .add(loc);
                        });
            }
        }
    ...
    public final void registerTick(SlimefunItem that) {
        that.addItemHandler(new BlockTicker() {
            int tickCount = 0;
    ...
    private void doParticle(Location loc) {
        CompletableFuture.runAsync(() -> {
            dv.forEach((v) -> {
                ...
                particleLocation
                        .getWorld()
                        .spawnParticle(getParticle(), particleLocation, 0, 0.0, 0.0, 0.0, 1, null, false);
            });
        });
    }

...
public abstract class AbstractSequenceProcessor extends AbstractMachine
        implements MachineProcessHolder<SequenceCraftingOperation> {
    protected final int[] BORDER =
            new int[] {0, 1, 2, 3, 4, 5, 6, 7, 8, 12, 21, 30, 14, 23, 32, 31, 36, 37, 38, 39, 40, 41, 42, 43, 44};
    protected final int[] BORDER_IN = new int[] {9, 10, 11, 18, 20, 27, 28, 29};
    protected final int[] BORDER_OUT = new int[] {15, 16, 17, 24, 26, 33, 34, 35};
    ...
    protected ItemStack INFO_ITEM = new CustomItemStack(
            Material.ORANGE_STAINED_GLASS_PANE,
            "&6机制",
            "&b该机器类型用于合成阶段输入配方",
            "&b放入配方序列的第一个物品开启配方",
            "&b按顺序满足配方每阶段输入需求才会进入下一阶段",
            "&b完成全部阶段机器会尝试输出结果");

    public AbstractSequenceProcessor(
            ItemGroup category,
        ...
        if (customRecipes != null) {
            ...
            for (var recipePiece : customRecipes2) {
                ...
                this.machineRecipes.add(MachineRecipeUtils.sequenceFrom(
                        2 * (int) recipePiece.getSecondValue(),
    ...
    public void process(Block b, BlockMenu inv, SlimefunBlockData data) {
        ...
        if (currentOperation == null) {
            Pair<MachineRecipe, ItemConsumer> nextP = CraftUtils.findNextSequenceRecipe(
                    inv, getInputSlots(), getMachineRecipes(), true, Settings.SEQUNTIAL, CRAFT_PROVIDER, clear != 0);
            if (nextP == null) {
                if (inv.hasViewer()) {
        ...
        if (currentOperation.isFinished()) {
            ...
            if (fastNext.isDirty()) currentOperation.progress(1);
            if (inv.hasViewer()) {

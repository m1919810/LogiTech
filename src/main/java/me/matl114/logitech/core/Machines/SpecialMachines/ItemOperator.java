...
public class ItemOperator extends AbstractMachine {
    protected final int[] INPUT_SLOTS = new int[] {28};
    protected final int[] OUTPUT_SLOTS = new int[] {10};
    protected final int[] BORDER_OUTPUT = new int[] {0, 1, 2, 9, 11, 18, 19, 20, 27, 29, 36, 37, 38};
    protected final int[] BORDER_INPUT =
            new int[] {3, 4, 5, 6, 7, 8, 12, 17, 21, 22, 23, 24, 25, 26, 30, 35, 39, 40, 41, 42, 43, 44};
    protected final int[] FUNCTION_SLOT = new int[] {13, 14, 15, 16, 31, 32, 33, 34};
    protected final int CONSUMED_DISPLAY_SLOT = 37;
    protected final int OPERATED_DISPLAY_SLOT = 1;
    protected final ItemStack CONSUMED_DISPLAY_ITEM =
            new CustomItemStack(Material.FURNACE, "&6消耗物品槽", "&7将操作所需求的物品置于该槽位");
    ...
    protected final ItemStack[] FUNCTION_ITEM = new ItemStack[] {
        ...
        new CustomItemStack(
                ...
                "&6赋予无法破坏",
                "&7点击赋予物品无法破坏属性",
                "&7需消耗 [%s] x64".formatted(Language.get("Items.LSINGULARITY.Name")),
                "&e一次仅可对一个物品操作"),
        new CustomItemStack(Material.GRINDSTONE, "&6重置物品", "&7点击重置原版物品", "&7无需消耗物品", "&7仅限原版物品使用"),
        new CustomItemStack(
                Material.WRITABLE_BOOK, "&6去除物品名字", "&7点击去除物品名字", "&7需消耗 [命名牌]x1", "&a粘液物品将恢复至原初的名字", "&e一次仅可对一个物品操作"),
        ...
        inv.addMenuClickHandler(FUNCTION_SLOT[0], ((player, i, itemStack, clickAction) -> {
            ...
            AddUtils.asyncWaitPlayerInput(player, (string) -> {
                ...
                if (consumed != null && CraftUtils.matchItemStack(consumed, NAMETAG, true)) {
                    ...
                    if (isSlimefunItem) {
                        meta.setDisplayName(
                                AddUtils.resolveColor(AddUtils.concat(RENAME_SF_PREFIX, string, RENAME_SF_SUFFIX)));
                    } else {
                        meta.setDisplayName(AddUtils.resolveColor(
                                AddUtils.concat(RENAME_VANILLA_PREFIX, string, RENAME_VANILLA_SUFFIX)));
                    }

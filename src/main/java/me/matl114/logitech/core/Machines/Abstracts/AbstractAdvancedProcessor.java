...
public abstract class AbstractAdvancedProcessor extends AbstractMachine
        implements MachineProcessHolder<MultiCraftingOperation> {
    protected final int[] BORDER = {3, 4, 5, 12, 13, 14, 21, 22, 23, 30, 31, 32, 39, 40, 41, 48, 49, 50};
    protected final int[] BORDER_SLOT = {2, 6, 11, 15, 20, 24, 29, 33, 38, 42, 47, 51};
    protected final int[] INPUT_SLOT = {0, 1, 9, 10, 18, 19, 27, 28, 36, 37, 45, 46};
    protected final int[] OUTPUT_SLOT = {7, 8, 16, 17, 25, 26, 34, 35, 43, 44, 52, 53};
    ...
    public AbstractAdvancedProcessor(
            ItemGroup category,
    ...
    public void addInfo(ItemStack stack) {
        ...
        stack.setItemMeta(AddUtils.machineInfoAdd(
                        AddUtils.addLore(stack, "&8⇨ &7并行处理机器"), this.energybuffer, this.energyConsumption)
                .getItemMeta());
    }
    /**
    ...
    public void process(Block b, BlockMenu inv, SlimefunBlockData data) {
        ...
        if (currentOperation == null) {
            ...
            Pair<MachineRecipe, ItemGreedyConsumer[]> nextQ = CraftUtils.matchNextMultiRecipe(
                    inv,
                    getInputSlots(),
                    getMachineRecipes(data),
                    USE_HISTORY,
                    maxCraftlimit,
                    Settings.SEQUNTIAL,
                    CRAFT_PROVIDER);
            ...
            ItemGreedyConsumer[] nextP = CraftUtils.countMultiOutput(
                    nextQ.getSecondValue(), inv, getOutputSlots(), next, craftlimit, CRAFT_PROVIDER);
            if (nextP != null) {

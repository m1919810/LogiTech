...
public class SimulateTestGenerator extends AbstractMachine {
    ...
    protected final int[] OUTPUT_SLOTS = new int[] {2, 3, 4, 5, 6, 7, 8};
    ...
    public SimulateTestGenerator(
        ...
        this.setDisplayRecipes(Utils.list(
                AddUtils.getInfoShow(
                        ...
                        "&7该机器会模拟虚空量子发电机的行为",
                        "&7在左侧机器槽插入能源调节器",
                        "&7机器每次运行会模拟%d*log_2(2*<调节器>)次运行".formatted(MULTIPLE_TIME),
                        "&7当机器中电力位于%dJ~%dJ时".formatted(outputMin, outputMax),
                        "&7清空电力并生成"),
                OUTPUT,
                this.MACHINE_COUNTER.getItem(),
                null));
        ...
        if (CraftUtils.matchItemStack(stack, MACHINE_COUNTER, false)) {
            ...
            if (inv.hasViewer()) {
                inv.replaceExistingItem(
                        INFO_SLOT, AddUtils.getGeneratorDisplay(true, "虚空量子", charge, this.energybuffer));
            }
        } else {
            if (inv.hasViewer()) {

...
public class BiReactor extends AbstractEnergyProvider {
    protected final int[] BORDER = new int[] {3, 4, 5, 12, 14, 21, 22, 23};
    protected final int[] INPUT_BORDER = new int[] {0, 2, 9, 11, 18, 19, 20};
    protected final int[] OUTPUT_BORDER = new int[] {6, 8, 15, 17, 24, 25, 26};
    protected final int[] INPUT_SLOTS = new int[] {10, 16};
    protected final int[] TRUE_SLOTS = new int[] {16};
    protected final int[] FALSE_SLOTS = new int[] {10};
    protected final int[] INFO_SLOT = new int[] {1, 7};
    ...
    protected final ItemStack[] INFO_ITEMS = new ItemStack[] {
        new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE, "&6[%s]输入槽".formatted(Language.get("Items.FALSE_.Name"))),
    ...
    public BiReactor(
        ...
        STATUS_ITEM[0] = AddUtils.addGlow(new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE, "&e状态: &a正", "&7发电量: %dJ/t".formatted(this.energyConsumption)));
        STATUS_ITEM[1] = AddUtils.addGlow(new CustomItemStack(
                Material.RED_STAINED_GLASS_PANE, "&e状态: &c负", "&7发电量: %dJ/t".formatted(-this.energyConsumptionFalse)));
        this.setDisplayRecipes(Utils.list(
                AddUtils.getInfoShow(
                        "&f机制 &a正",
                        "&7当同时输入%s和%s时,视为满足条件,发电 %dJ"
                                .formatted(
                                        Language.get("Items.FALSE_.Name"),
                                        Language.get("Items.TRUE_.Name"),
                                        this.energyConsumption)),
                new DisplayItemStack(AddItem.TRUE_),
                AddUtils.getInfoShow(
                        "&f机制 &c负", "&7当不满足’正‘的输入条件时候,视为不满足条件,发电 -%dJ".formatted(this.energyConsumptionFalse)),
    ...
    public int getGeneratedOutput(@Nonnull Location l, @Nonnull SlimefunBlockData data) {
        ...
        if (inv != null) {
            ...
            if (it != null
                    && it.getType() == Material.MUSIC_DISC_5
                    && (!it.getEnchantments().isEmpty())) {
                ItemStack it2;
                it2 = inv.getItemInSlot(FALSE_SLOTS[0]);
                if (it2 != null
                        && it2.getType() == Material.MUSIC_DISC_5
                        && (it2.getEnchantments().isEmpty())) {
                    it.setAmount(0);
                    it2.setAmount(0);
                    if (inv.hasViewer()) {

...
public class TntGenerator extends AbstractMachine {
    ...
    protected final int[] BORDER = new int[] {1, 2, 3, 5, 6, 7};
    protected final int START_SLOT = 0;
    protected final ItemStack START_ITEM_ON =
            new CustomItemStack(Material.GREEN_STAINED_GLASS_PANE, "&6当前状态", "&a已启动", "&e点击关闭");
    protected final ItemStack START_ITEM_OFF =
            new CustomItemStack(Material.RED_STAINED_GLASS_PANE, "&6当前状态", "&a已关闭", "&e点击启动");
    ...
    public TntGenerator(ItemGroup category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        ...
        PostSetupTasks.addPostRegisterTask(() -> Schedules.launchSchedules(gtThread, 100, false, 1));
    }
    ...
    protected final ConcurrentHashMap<Chunk, ConcurrentHashMap<Location, BlockMenu>> INSTANCES =
    ...
    public ItemStack getDelayDisplay(DataMenuClickHandler dh) {
        return new CustomItemStack(
                Material.CLOCK, "&6生成频率信息", "&7生成间隔: %dgt".formatted(dh.getInt(2)), "&e左键 增加延时 1gt", "&e右键 减少延时 1gt");
    }
    ...
    public void newMenuInstance(BlockMenu menu, Block block) {
        ...
        menu.addMenuClickHandler(START_SLOT, ((player, i, itemStack, clickAction) -> {
            ...
            menu.getLocation()
                    .getBlock()
                    .setBlockData(
                            menu.getLocation().getBlock().getBlockData(), true); //   .getState().update(true,true);
            return false;
        }));
        ...
        menu.addMenuClickHandler(ADJUSTMENT_SLOT, ((player, i, itemStack, clickAction) -> {
            ...
            switch (code) {
                case 0:
                    dh2.setInt(0, dh2.getInt(0) + 1);
                    break;
                case 1:
                    dh2.setInt(0, dh2.getInt(0) - 1);
                    break;
                case 2:
                    dh2.setInt(1, dh2.getInt(1) + 1);
                    break;
                case 3:
                    dh2.setInt(1, dh2.getInt(1) - 1);
                    break;

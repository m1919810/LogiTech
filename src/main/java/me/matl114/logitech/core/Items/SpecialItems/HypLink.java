                    ...
                    if (WorldUtils.hasPermission(
                        ...
                        if (data != null) {
                            ...
                            if (menu != null && item != null) {
                                if (item.canUse(event.getPlayer(), true)
                                        && menu.canOpen(loc.getBlock(), event.getPlayer()))
                                    menu.open(event.getPlayer());
                                else AddUtils.sendMessage(event.getPlayer(), "&c打开该粘液方块的行为被阻止!");
                                return;
                            }
                        }
                        Block b = loc.getBlock();
                        if (me.matl114.matlib.utils.WorldUtils.isInventoryHolder(b.getType())) {
                            PlayerInteractEvent interactTarget = new InteractPermissionTestEvent(
                                    event.getPlayer(), Action.RIGHT_CLICK_BLOCK, null, b, BlockFace.UP);
                            try {

...
public class EntityFeat extends CustomItemWithHandler<ItemDropHandler> {
    ...
    public static ItemStack getItemFromEntityType(EntityType entityType) {
        ...
        if (!hasTypeLore) {
            ...
            lore.add("&7类型: "
                    + AddUtils.color(
                            (entityType == null || entityType == EntityType.UNKNOWN)
                                    ? "类型"
                                    : ChatUtils.humanize(entityType.name())));
        }
     ...
     * @return
     */
    public static final String SPAWNER_ATTRIBUTE = AddUtils.resolveColor(AddUtils.color("刷怪笼属性:"));
    ...
    private static final boolean enableSuperSpawnerCraft =
            entityFeat.getOrSetDefault("enable-super-spawner-craft", true);
    private static final HashSet<EntityType> availableSpawnerTypes = new HashSet<>() {
        {
            ...
            List<String> val = entityFeat.getOrSetDefault(
                    "banned-spawner-types",
                    List.of(
                            "snowball",
                            "small_fireball",
                            "fireball",
    ...
    public static ItemStack generateSpawnerFrom(
            EntityType type,
            int delay,
            int maxNearbyEntities,
            int requirePlayerRange,
            int spawnRange,
            int spawnCount,
    ...
    public boolean onItemDrop(PlayerDropItemEvent var1, Player var2, Item var3) {
        if (isItem(var3.getItemStack())) {
            if (canUse(var2, true)) {
                Schedules.launchSchedules(() -> craftToSpawmers(var2, var3), 30, true, 0);
            }
            return true;
        }
        return false;
    ...
    public ItemDropHandler[] getItemHandler() {
        return new ItemDropHandler[] {(ItemDropHandler) this::onItemDrop};
    }

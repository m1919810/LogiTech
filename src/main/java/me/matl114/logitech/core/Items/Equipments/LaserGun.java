...
public class LaserGun extends ChargableProps {
        ...
        if (lores.size() > 1) {
            lores.set(
                    ...
                    AddUtils.concat(
                            ...
                            "档/共",
                            String.valueOf(rankAmount.getValue()),
                            "档"));
        }
        meta.setLore(lores);
    }
    ...
    public void addInfo(ItemStack stack) {
        stack.setItemMeta(AddUtils.insertLore(
                        stack,
                        -3,
                        "&7射程: %dm".formatted(distance.getValue()),
                        "&7伤害: 100*<功率档>(对怪物) 或 %d*<功率档>级激光伤害(对玩家)".formatted(lazerLevel.getValue()))
                .getItemMeta());
    ...
    public void onLaser(Player p, ItemStack stack, boolean onHead, boolean withWarning) {
        ...
        if (charge >= consumption) {
            ...
            AddUtils.sendMessage(
                    p, AddUtils.concat("&c电力不足! ", String.valueOf(charge), "J/", String.valueOf(consumption), "J"));
            // }
        }
    }
    ...
    private void launchLaser(Player p, int level, boolean fromEye) {
        ...
        final Pair<Integer, Location> endLocaion =
                WorldUtils.rayTraceLocation(p.getLocation().getDirection(), loc, 0.25, 100, new Predicate<Location>() {
                    ...
                    public boolean test(Location location) {
                        if (testIsAir(location)) {
                            ...
                            Collection<Entity> entities =
                                    location.getWorld().getNearbyEntities(location, 0.4, 0.4, 0.4);
                            if (!entities.isEmpty()) {
                                    ...
                                    if (entity instanceof Player player) {
                                        if (p != player) {
                                            ...
                                            if (true || WorldUtils.testAttackPermission(p, player, damage)) {
                                                PlayerEffects.grantEffect(
                                                        CustomEffects.LASER, player, 4 * (level + 1), 1);
                                            }
                                        }
                                    } else {

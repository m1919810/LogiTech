...
public class DisplayUseTrimmerLogicLate extends MaterialItem implements InterfacedSmithTableTrimmer {
    ...
    public DisplayUseTrimmerLogicLate(
            ...
            Supplier<Set<ItemStack>> trimmerItemLatelyInitFunc,
            Supplier<RecipeChoice> baseItemLatelyInitFunc,
            Supplier<RecipeChoice> extraItemLatelyInitFunc,
            Function<SmithingInventory, Triplet<ItemStack[], Integer, Integer>> trimmerLogic) {
    ...
    public RecipeChoice getTrimmerRecipeChoice() {
        if (trimmerChoice == null) {
            trimmerChoice = new RecipeChoice.ExactChoice(
                    trimmerItems.get().stream().filter(Objects::nonNull).toList());
        }

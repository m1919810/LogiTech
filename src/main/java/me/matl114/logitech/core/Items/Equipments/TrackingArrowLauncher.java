...
public class TrackingArrowLauncher extends ChargableProps {
    ...
    public TrackingArrowLauncher(
            ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        super(itemGroup, item, recipeType, recipe);
    }

    public void addInfo(ItemStack stack) {
        stack.setItemMeta(AddUtils.insertLore(
                        stack,
                        -1,
                        "&7寻踪范围: %s".formatted(String.valueOf(trackRange.getValue())),
                        "&7穿透数: 4",
                        "&7箭矢伤害: %s+%s*<力量等级>+%s*<锋利等级>"
                                .formatted(
                                        String.valueOf(basicDamage.getValue()),
                                        String.valueOf(powerAmplifier.getValue()),
                                        String.valueOf(sharpnessAmplifier.getValue())))
                .getItemMeta());
        super.addInfo(stack);
    }
        ...
        if (p != null) {
            ...
            if (charge < ENERGYCONSUMPTION) {
                AddUtils.sendMessage(
                        p,
                        AddUtils.concat(
                                "&c电力不足! ", String.valueOf(charge), "J/", String.valueOf(ENERGYCONSUMPTION), "J"));
            } else {
                if (canUse(p, false)) {
    ...
    public void onArrowLaunched(Player p, ItemStack item) {
        ...
        HashSet<Entity> targetEntities =
                WorldUtils.getEntityInDistance(targetCenter, 1.2 * trackRange.getValue(), (e) -> {
                    return e != p && WorldUtils.isTargetableLivingEntity(e);
                });
        // 索敌算法
    ...
    public Arrow spawnArrowRandomly(Location loc, Player p, float damage) {
        ...
        Location loc2 = loc.clone()
                .add(
                        rand.nextFloat(-LOCATION_DELTA, LOCATION_DELTA),
                        rand.nextFloat(-LOCATION_DELTA / 4, 0),
                        rand.nextFloat(-LOCATION_DELTA, LOCATION_DELTA));
        // 10度的偏差
                    ...
                    try {
                        ...
                        for (Arrow arrow : arrows) {
                            ...
                            if (runTime % 2 == 0 && runTime > 5)
                                arrow.getWorld()
                                        .spawnParticle(
                                                Particle.FLAME, arrow.getLocation(), 0, 0.0, 0.0, 0.0, 1, null, false);
                        }
                    } finally {
                        isRunning = false;
                    }
    ...
    public void retrackArrow(Arrow arrow, LivingEntity target) {
        ...
        if (loc.getWorld() == targetLoc.getWorld() && loc.distance(targetLoc) > 1.0f) {
            arrow.setVelocity(targetLoc
                    .subtract(loc)
                    .toVector()
                    .normalize()
                    .multiply(ARROW_SPEED * DEFLECTION_RATE)
                    .add(arrow.getVelocity().multiply(1.0f - DEFLECTION_RATE)));
        } else {

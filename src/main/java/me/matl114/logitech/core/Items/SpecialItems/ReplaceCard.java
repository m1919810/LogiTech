...
public class ReplaceCard extends DataRecordedItem {
    public static final String LOC_DISPLAY_PREFIX = AddUtils.resolveColor("&x&E&B&3&3&E&B替代的物品: &f");
    public static final NamespacedKey KEY_LOC = AddUtils.getNameKey("rep_mat_id");
    ...
    public ReplaceCard(
            ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, ReplaceType type) {
        super(itemGroup, item, recipeType, recipe);
        this.setDisplayRecipes(Utils.list(AddUtils.getInfoShow("&f机制", "&7可以在快捷机器中替代其中记录的不可堆叠物品", "&7桶类物品会在合成之后返还铁桶")));
        this.type = type;
        instances.add(this);
    }

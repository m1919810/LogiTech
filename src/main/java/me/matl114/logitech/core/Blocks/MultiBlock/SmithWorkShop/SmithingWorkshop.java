...
public class SmithingWorkshop extends MultiCore {
    ...
    protected final ItemStack STATUS_ITEM_ON =
            new CustomItemStack(Material.GREEN_STAINED_GLASS_PANE, "&6锻造核心信息", "&7工坊状态: &a开启", "&7自动构建:强制开启");
    protected final ItemStack STATUS_ITEM_OFF = new CustomItemStack(
            Material.RED_STAINED_GLASS_PANE, "&6锻造核心信息", "&c点击查看结构错误", "&7工坊状态: &c关闭", "&7自动构建:强制开启");
    ...
    protected final ItemStack PLUGIN_ITEM =
            new CustomItemStack(Material.ORANGE_STAINED_GLASS_PANE, "&6工坊增幅组件槽", "&7于下方插入工坊增幅组件");
    ...
    public SmithingWorkshop(
        ...
        this.setDisplayRecipes(Utils.list(AddUtils.getInfoShow("&f机制 - &c多方块机器", "&7")));
    }
    ...
    public void listenOriginClick(BlockMenu inv, InventoryClickEvent e) {
        ...
        if (e.getClick() == ClickType.SHIFT_LEFT || e.getClick() == ClickType.SHIFT_RIGHT) {
            if (e.getRawSlot() >= inv.getInventory().getSize()) {
                ...
                if (SWAmplifyComponent.getComponentType(stack) != null
                        && SlimefunItem.getByItem(stack) == SWAmplifyComponent.getComponentType(stack)) {
                    for (int i : PLUG_SLOT) {
            ...
            inv.addMenuClickHandler(i, ((player, i1, itemStack, clickAction) -> {
                if (itemStack != null && clickAction.isShiftClicked()) {
                    ...
                    if (leftOver.get(0) != null && leftOver.get(0).getAmount() == amount) {
                        ...
                        int realAmount =
                                leftOver.get(0) == null ? 0 : leftOver.get(0).getAmount();
    ...
    public int getAmplifyCompentLevel(Location loc, SWAmplifyComponent component) {
        ...
        return (int) Arrays.stream(PLUG_SLOT)
                .filter(i -> {

...
public class SolarReactorCore extends MultiBlockProcessor {
    ...
    protected final ItemStack HOLOGRAM_ITEM_ON = new CustomItemStack(
            SlimefunItems.HOLOGRAM_PROJECTOR, "&6点击切换全息投影", "&e或使用/logitech multiblock 查看搭建教程", "&7当前状态: &a开启");
    protected final ItemStack HOLOGRAM_ITEM_OFF = new CustomItemStack(
            SlimefunItems.HOLOGRAM_PROJECTOR, "&6点击切换全息投影", "&e或使用/logitech multiblock 查看搭建教程", "&7当前状态: &c关闭");
    ...
    public SolarReactorCore(
        ...
        super(
                ...
                blockId,
                type,
                AddUtils.addGlow(new ItemStack(Material.FIRE_CHARGE)),
                energyConsumption,
                energyBuffer,
                customRecipes);
    ...
    public void onMultiBlockDisable(
            Location loc, AbstractMultiBlockHandler handler, MultiBlockService.DeleteCause cause) {
        removeEffect(loc);
    ...
    public void removeEffectSync(Location loc) {
        ...
        BukkitUtils.executeSync(() -> {
            if (cry != null) cry.remove();
            Collection<Entity> allCrystal = loc2.getWorld()
                    .getNearbyEntities(
                            loc2, REMOVE_EFFECT_OFFSET, REMOVE_EFFECT_OFFSET, REMOVE_EFFECT_OFFSET, (entity -> {
    ...
    public void removeEffect(Location loc) {
        ...
        //            if(cry!=null)
        //                cry.remove();
        //            Collection<Entity> allCrystal=loc2.getWorld().getNearbyEntities(loc2, REMOVE_EFFECT_OFFSET,
        // REMOVE_EFFECT_OFFSET, REMOVE_EFFECT_OFFSET,(entity -> {
        //                return entity.getType()==EntityType.ENDER_CRYSTAL&&checkBind(entity,loc);
        //            }));
        //            for (Entity entity:allCrystal){
        //                WorldUtils.executeOnSameEntity(entity,(entity1 -> entity1.remove()));
    ...
    public static class NotFilledWithAirCause extends MultiBlockService.DeleteCause {
        ...
        public NotFilledWithAirCause(Block block) {
            super(
                    AddUtils.concat(
                            "超新星没有被空气包裹,在",
                            DataCache.locationToDisplayString(block.getLocation()),
                            "处发现了",
    ...
    public boolean checkCondition(Location loc, SlimefunBlockData data) {
        ...
        Schedules.launchSchedules(
                Schedules.getRunnable(() -> {
                    Location loc1 = loc.clone().add(0.5, 1, 0.5);
                    Collection<Entity> allEntities = loc1.getWorld()
                            .getNearbyEntities(loc1, ENTITY_KILL_OFFSET, ENTITY_KILL_OFFSET, ENTITY_KILL_OFFSET);
                    for (Entity entity : allEntities) {
                        if (entity instanceof EnderCrystal) {

                        } else if (entity instanceof Player player) {
                            PlayerEffects.grantEffect(CustomEffects.SOLAR_BURN, player, 2, 3, (player1 -> {
                                return loc1.distance(player1.getLocation()) < ENTITY_KILL_OFFSET + 1;
    ...
    public void onDestroyEffect(Location loc, AbstractMultiBlockHandler handler, MultiBlockService.DeleteCause cause) {
        ...
        if (broadCastWhenDump.getValue()) {
            AddUtils.broadCast("&e位于[%s,%.0f,%.0f,%.0f]的超新星模拟器因 [%s] 即将爆炸,快跑!"
                    .formatted(loc.getWorld().getName(), loc.getX(), loc.getY(), loc.getZ(), cause.getMessage()));
        } else {
        ...
        Schedules.launchSchedules(
                () -> {
                    ...
                    Collection<Entity> entities = center.getWorld()
                            .getNearbyEntities(
                                    center, ENTITY_EFFECT_OFFSET, ENTITY_EFFECT_OFFSET, ENTITY_EFFECT_OFFSET);
            ...
            if (clickAction.isShiftClicked()) {
                ...
                if (statusCode == 0) {
                    if (this.getCharge(loc) < energyConsumption) {
                        ...
                        if (MultiBlockService.createNewHandler(
                                loc, getBuilder(), getMultiBlockType(), OutputStream.getPlayerOut(player))) {
    ...
    public void tick(Block b, BlockMenu inv, SlimefunBlockData data, int tickCount) {
        ...
        if (statusCode == 0) {
            ...
            if (charge < energyConsumption) {
                ...
                MultiBlockService.deleteMultiBlock(
                        MultiBlockService.safeGetUUID(loc),
                        MultiBlockService.EnergyOutCause.get(charge, energyConsumption));
                return;
            }
            if (inv.hasViewer()) {
                ...
                if (it != null) {
                    ...
                    AddUtils.setLore(
                            it,
                            "&7当前机器状态: %s".formatted((currentOperation == null) ? "&7待机中" : "&a进程中"),
                            "&7当前电量: %dJ/%dJ".formatted(charge, energybuffer),
                            "&7当前耗电: %dJ/t".formatted(energyConsumption));
                } else inv.replaceExistingItem(INFO_SLOT, INFO_ITEM);
            }

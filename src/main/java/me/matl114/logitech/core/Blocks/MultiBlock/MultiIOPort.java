...
public class MultiIOPort extends AbstractMachine implements MultiBlockPart {
    ...
    public MultiIOPort(
        ...
        this.INFO_ITEM = new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE,
                "&a传输器信息",
                AddUtils.concat("&7类别: &6", CONFIG_TO ? "输入接口" : "输出接口"),
            ...
            if (inv != null) {
                if (CONFIG_TO) {
                    TransportUtils.transportItem(
                            menu, getOutputSlots(), inv, TransportUtils.getInvInputSlot(inv), CONFIG_SYMM, null, 576);
                } else {
                    TransportUtils.transportItem(
                            inv, TransportUtils.getInvOutputSlot(inv), menu, getInputSlots(), CONFIG_SYMM, null, 576);
                }
            }
        }
    }

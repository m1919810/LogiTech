...
public class StorageLink {
    public static final String LOC_DISPLAY_PREFIX = AddUtils.resolveColor("&x&E&B&3&3&E&B纠缠链接目标: ");
    public static final String NAME_DISPLAY_PREFIX = AddUtils.resolveColor("&x&E&B&3&3&E&B历史访问记录: ");
    public static final String BLANK_COLOR = AddUtils.resolveColor("&f");
    public static final String BLANK_STORAGE = AddUtils.resolveColor("&f空存储位");
    public static final NamespacedKey KEY_LOC = AddUtils.getNameKey("cache_loc");
    public static final String LOC_DISPLAY = AddUtils.resolveColor("&3[%s,x=%d,y=%d,z=%d]");
    ...
    public static Location getLink(ItemMeta meta) {
        try {
            return DataCache.locationFromString(
                    meta.getPersistentDataContainer().get(KEY_LOC, PersistentDataType.STRING));
        } catch (Throwable e) {
            return null;
        }
    }
    ...
    public static void updateLore(List<String> lore, Location loc) {
        lore.add(AddUtils.concat(
                LOC_DISPLAY_PREFIX,
                LOC_DISPLAY.formatted(loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ())));

...
public abstract class AbstractFilter extends AbstractBlock implements Ticking {
    ...
    public ItemStack CLEAN_ITEM_OFF =
            new CustomItemStack(SlimefunItems.TRASH_CAN, "&3开启输入槽清空模式", "&7当输入槽物品无法匹配输出槽白名单时,会清空输入槽", "&7当前状态: &c关闭");
    public ItemStack CLEAN_ITEM_ON =
            new CustomItemStack(SlimefunItems.TRASH_CAN, "&3关闭输入槽清空模式", "&7当输入槽物品无法匹配输出槽白名单时,会清空输入槽", "&7当前状态: &a开启");
    ...
    public void tick(Block b, BlockMenu inv, SlimefunBlockData data, int tickCount) {
        ...
        ItemStack stack;
        DynamicArray<ItemConsumer> wlCounters = new DynamicArray<>(
                ItemConsumer[]::new,
                outputSlots.length,
                (i) -> CraftUtils.getConsumer(inv.getItemInSlot(outputWLSlot[i])));
        for (int i = 0; i < len; i++) {
            ...
            if (!hasTransported) {
                if (code.length() == 1) {
                    ...
                    TransportUtils.transportItem(
                            ...
                            nowInputSlot,
                            inv,
                            new int[] {outputSlots[outputSlots.length - 1]},
                            CargoConfigs.getDefaultConfig(),
                            false,
                            null,
                            CraftUtils.getpusher);

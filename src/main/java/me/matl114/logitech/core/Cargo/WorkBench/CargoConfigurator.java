...
public class CargoConfigurator extends AbstractBlock {
    protected final int[] BORDER = new int[] {0, 1, 2, 3, 5, 6, 7, 8, 27, 28, 34, 35};
    protected final int[] INFO_SLOTS = new int[] {4, 9, 10, 11, 12, 13, 14, 15, 16, 17, 30, 32};
    protected final int[] CONFIG_SLOTS = new int[] {18, 19, 20, 21, 22, 23, 24, 25, 26, 29, 33};
    ...
    protected final ItemStack[] INFO_ITEMS = new ItemStack[] {
        ...
        new CustomItemStack(
                ...
                "&6配置 强对称 的配置槽位",
                "&e将[%s]或[%s]置于下方".formatted(Language.get("Items.TRUE_.Name"), Language.get("Items.FALSE_.Name")),
                "&7配置是否使用强对称传输",
                "&7即是否将物品按槽位进行对应的运输"),
        new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE,
        ...
        new CustomItemStack(
                ...
                "&e将[%s]或[%s]置于下方".formatted(Language.get("Items.TRUE_.Name"), Language.get("Items.FALSE_.Name")),
                "&7配置源方块模式(抓取)",
                "&7若该项为True,则优先从源方块输入槽进行货运",
                "&7蕴含最终源方块模式 &e输入槽 &7和 &e输入槽+输出槽",
                "&7若该项为False,则优先从源方块输出槽进行货运",
                "&7蕴含最终源方块模式 &e输出槽 &7和 &e输出槽+输入槽"),
        new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE,
                "&6配置 目标方块槽位优先级 的配置槽位",
                "&e将[%s]或[%s]置于下方".formatted(Language.get("Items.TRUE_.Name"), Language.get("Items.FALSE_.Name")),
                "&7配置目标方块模式(推送)",
                "&7若该项为True,则优先从目标方块输出槽进行货运",
                "&7蕴含最终目标方块模式 &e输出槽 &7和 &e输出槽+输入槽",
                "&7若该项为False,则优先从目标方块输入槽进行货运",
                "&7蕴含最终目标方块模式 &e输入槽 &7和 &e输入槽+输出槽"),
        new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE,
                "&6配置 源方块++ 的配置槽位",
                "&e将[%s]或[%s]置于下方".formatted(Language.get("Items.TRUE_.Name"), Language.get("Items.FALSE_.Name")),
                "&7配置是否启用源方块++模式",
                "&7该模式为True时,货运会同时涉及输入槽和输出槽",
                "&7++模式且抓取输入槽优先:True时,最终源方块模式为 &e输入槽+输出槽",
        ...
        new CustomItemStack(
                ...
                "&7配置是否启用目标方块++模式",
                "&7该模式为True时,货运会同时涉及输入槽和输出槽",
                "&7++模式且推送输出槽优先:True时,最终目标方块模式为 &e输出槽+输入槽",
                "&7++模式且推送输出槽优先:False,最终目标方块模式为 &e输入槽+输出槽"),
        new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE,
                "&6配置 传输数量限制x64 的配置槽位",
                "&e将[%s]置于左侧".formatted(Language.get("Items.CARGO_PART.Name")),
                "&7配置传输数量",
                "&a传输数量增加<物品数量>*64"),
        new CustomItemStack(
                Material.GREEN_STAINED_GLASS_PANE,
                "&6配置 传输数量限制x1 的配置槽位",
                "&e将[%s]置于右侧".formatted(Language.get("Items.CARGO_PART.Name")),
                "&7配置传输数量",
                "&a传输数量增加<物品数量>"),
    };
    protected final ItemStack TIPS_ITEM = AddUtils.randItemStackFactory(new PairList() {
        {
            ...
            put(
                    AddUtils.addGlow(new CustomItemStack(
                            Material.KNOWLEDGE_BOOK, "&x&E&B&3&3&E&B小彩蛋", "&b恭喜您抽中了1/3600概率的彩蛋", "&b想来您今天的运势一定很好!")),
                    1);
        }
    });

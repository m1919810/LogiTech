...
public class StorageSpace {
    ...
    public static Pair<Integer, Location> generateNewLocation(int sizeX, int sizeY, int sizeZ) {
        ...
        dx *= STORAGE_DISTANCE;
        dz *= STORAGE_DISTANCE;
        ConfigLoader.SPACE_STORAGE.setValue(
                "storages." + num + ".loc",
                String.join(
                        ",",
                        String.valueOf(dx),
    ...
    public static void onSpaceExchange(
            Player player,
        ...
        BukkitUtils.executeAsync(
                () -> onSpaceExchangeAsync(player, startLocation, endLocation, targetLocation, status, checkPerms));
    }
    ...
    // menu内含物掉落和这些操作在一起
    // todo need test
    private static void onSpaceExchangeAsync(
            Player player,
            Location startLocation,
            Location endLocation,
            Location storeStartLocation,
            int status,
            boolean checkPerms) {
        ...
        for (int i = chunkx; i <= chunkx_; ++i) {
            for (int j = chunkz; j <= chunkz_; ++j) {
                ...
                sfBlocklocations.addAll(
                        data.stream().map((datas) -> datas.getLocation()).toList());
            }
        }
                            ...
                            for (int j = 0; dy_ + j < y && j < 8; ++j) {
                                for (int k = 0; dz_ + k < z && k < 8; ++k) {
                                    Location originLocation =
                                            startLocation.clone().add(dx_ + i, dy_ + j, dz_ + k);
                                    // 先进行简单权限检测 剔除保护module里保护的内容
                                    // 这里保证100%没有粘液方块
                                    if (!sfBlocklocations.contains(originLocation)
                                            && (!checkPerms
                                                    || WorldUtils.hasPermission(
                                                            player,
                                                            originLocation,

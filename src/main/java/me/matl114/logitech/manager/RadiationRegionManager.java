...
public class RadiationRegionManager {
    ...
    public static boolean removeNearRadiation(Location loc, double radius) {
        synchronized (lock) {
            Iterator<Map.Entry<Location, BukkitRunnable>> it =
                    RADIATION_THREAD.entrySet().iterator();
            while (it.hasNext()) {
    ...
    public static void runRadiation(Location loc, int range, int level) {
        ...
        for (Entity entity : entities) {
            if (entity instanceof Player player) {
                ...
                PlayerEffects.grantEffect(CustomEffects.RADIATION, player, level, 60, (player1 -> true));
            }
        }
    }
}
